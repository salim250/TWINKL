...
export const Navigation = () => {
  ...
  const menuItems = [
    ...
    { label: t('nav.schedules') || 'Schedules', page: 'schedules' as const },
  ];
        ...
        <div className="max-w-content mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            ...
            >
              ...
              <span className="ml-3 text-xl font-heading font-bold text-primary hidden sm:block">
                TWINKL Education
              </span>
            </div>
            ...
            <div className="hidden xl:flex items-center space-x-6">
              ...
              {/* User Auth: Login / Logout Desktop */}
              {user ? (
                <button
                  onClick={() => signOut()}
              ...
              {menuItems.map((item) =>
                item.children ? (
                  ...
                  <button
                    key={item.page}
                    onClick={() => handleNavigate(item.page)}
                    className={`block w-full text-left px-4 py-2 rounded-lg text-sm hover:bg-gray-100 ${currentPage === item.page ? 'text-secondary font-bold' : ''
                      }`}
                  >
                    {item.label}
                  </button>
